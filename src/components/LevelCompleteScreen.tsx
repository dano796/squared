...
function Star({ index, earned }: { index: number; earned: boolean }) {
  ...
  return (
    <span style={{
      ...
      filter: earned && visible ? 'drop-shadow(0 0 6px rgba(201,150,58,0.5))' : 'none',
...
export default function LevelCompleteScreen() {
  ...
  return (
    <div
      ...
      style={{ background: 'linear-gradient(160deg, #12121c 0%, #0e0e14 60%, #101814 100%)' }}
    >
      <div style={{
        display: 'flex',
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {[0, 1, 2].map(i => <Star key={i} index={i} earned={i < stars} />)}
        </div>
        ...
        <div style={{
          ...
          <div style={rowStyle}>
            ...
            <span style={{ color: '#e6e4f0', fontWeight: 600 }}>{formatTime(time)}</span>
          </div>
          <div style={{ height: 1, background: '#1a281c' }} />
          <div style={rowStyle}>
