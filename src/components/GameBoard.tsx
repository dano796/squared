...
const FALL_DURATION = 1; // seconds for fall
...
function computeFallConfig(
  ...
  const ok = (x: number, y: number) =>
    (grid[y]?.[x] ?? 0) !== 0 && !brokenTiles.has(`${x},${y}`);
...
function getTipCorners(
  block: BlockState,
  cfg: Extract<FallConfig, { type: "tip" }>,
  ft: number,
...
function animCorners(anim: AnimState): [number, number, number][] {
  ...
  return blockCorners(anim.from).map(([c, r, z]) =>
    transformPt(c, r, z, anim, eased),
...
export default function GameBoard() {
  ...
  useEffect(() => {
    ...
    if (falling && !prevFallingRef.current) {
      ...
      if (isFalling) {
        ...
        fallConfigRef.current = computeFallConfig(
          storeBlock,
          grid,
          brokenTiles,
        );
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      // Straight fall runs in parallel with the roll; tip fall waits for roll to finish
      if (falling) {
        const isStraight = fallConfigRef.current.type === "straight";
        if (isStraight || !animRef.current) {
          fallTRef.current = Math.min(fallTRef.current + dt / FALL_DURATION, 1);
        }
      }

      // Shake — decays over 0.4 s driven by isShaking flag
      if (isShaking) {
        shakeTRef.current = Math.min(shakeTRef.current + dt / 0.4, 1);
      } else {
        shakeTRef.current = 0;
      }
      const shakeAmp = isShaking ? (1 - shakeTRef.current) * 5 : 0;
      const shakeX = shakeAmp * Math.sin(time * 60);
      const shakeY = shakeAmp * 0.4 * Math.cos(time * 55);

      const p = makeProj(TW, TH, ox + shakeX, oy + shakeY);
      ...
      for (const { col, row, tile } of tiles) {
        if (tile === 2) drawGoalTile(ctx, p, col, row, TW, TH, time);
          ...
          drawSwitchTile(ctx, p, col, row, tile, TW);
      ...
      if (falling) {
        ...
        if (cfg.type === "tip" && !animRef.current) {
          // Tip: roll completes first, then the block tips over the edge
          corners = getTipCorners(storeBlock, cfg, ft);
        } else if (cfg.type === "straight") {
          // Straight: gravity runs in parallel with the roll
          const gravity = -(ft * ft) * 6;
          if (animRef.current) {
            corners = animCorners(animRef.current).map(
              ([c, r, z]) => [c, r, z + gravity] as [number, number, number],
            );
          } else {
            corners = blockCorners(storeBlock, gravity);
          }
        } else {
          // Tip still mid-roll — show the roll normally
